class solver:
    ...
    def countContainedFor(self, bagColor):
        ...
        for contained in currentRule.getContained():
            ...
            result += (contained['count'] *
                       self.countContainedFor(contained['bag'].getColor()))
        return result
...
def parsePuzzleInputFrom(file):
    ...
    for line in file.readlines():
        ...
        while(not noMoreBags):
            ...
            delimiter, noMoreBags = getDelimiterForContainedBags(
                line, delimiter)
            bagColor, line = getBagColor(line, delimiter)
            currentRule.addContained(
                {'count': int(count), 'bag': bag(bagColor)})
